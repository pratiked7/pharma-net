...
TIMEOUT="$4"
VERBOSE="$5"
: ${CHANNEL_NAME:="pharmachannel"} #changed it from commonchannel to pharmachannel
: ${DELAY:="5"}
: ${LANGUAGE:="node"}
: ${TIMEOUT:="15"}
: ${VERBOSE:="false"}
...
COUNTER=1
MAX_RETRY=15
ORGS="manufacturer distributor retailer transporter"  #UPDATE REQUIRED
...
createChannel() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer channel create -o orderer.pharma.net:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
