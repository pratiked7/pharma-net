...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"upstacChannel\")"
  ...
  echo
  echo "	network.sh generate -c upstacChannel"
  echo "	network up -c upstacChannel -s couchdb"
  echo "        network up -c upstacChannel -s couchdb -i 1.4.0"
  echo "	network up -l node"
  echo "	network down -c upstacChannel"
  echo
  echo "Taking all defaults:"
  echo "	network generate"
  echo "	network up"
...
function clearContainers() {
  ...
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.commonchannel*/) {print $1}')
...
function removeUnwantedImages() {
  ...
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.commonchannel*/) {print $3}')
...
function networkUp() {
  ...
  IMAGE_TAG=$IMAGETAG docker-compose -f "$COMPOSE_FILE" up -d --force-recreate --renew-anon-volumes 2>&1
  docker ps -a
  if [ $? -ne 0 ]; then
  ...
  sleep 1
  echo "Sleeping 20s to allow cluster to complete booting"
  sleep 19
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/manufacturerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg manufacturerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/distributorMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg distributorMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/retailerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg retailerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/transporterMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg transporterMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  # echo "#################################################################"
  # set -x
  # configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/consumerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg consumerMSP
  # res=$?
  # set +x
  # if [ $res -ne 0 ]; then
...
CLI_DELAY=5
# channel name defaults to "upstacChannel"
